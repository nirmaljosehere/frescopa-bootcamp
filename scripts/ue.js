...
// Custom card-style blocks that share the boilerplate cards decorate() pattern
// (each authored <div> row becomes a <li>, and a single-picture cell becomes a
// `<name>-card-image`). They need the same UE instrumentation handling as the
// stock `cards` block, keyed off their own component name / image class.
const CARD_LIKE_BLOCKS = ['cards-tiles', 'cards-feature', 'cards-product', 'cards-numbered'];

const setupObservers = () => {
  const cardLikeSelector = CARD_LIKE_BLOCKS.map((b) => `div.${b}`).join(', ');
  const mutatingBlocks = document.querySelectorAll(`div.cards, div.carousel, div.accordion, ${cardLikeSelector}`);
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.type === 'childList' && mutation.target.tagName === 'DIV') {
        ...
        const componentName = mutation.target.attributes['data-aue-component']?.value;
        let type;
        if (targetClasses.contains('cards-card-image')
          || CARD_LIKE_BLOCKS.some((b) => targetClasses.contains(`${b}-card-image`))) {
          type = 'cards-image';
        } else if (componentName === 'cards' || CARD_LIKE_BLOCKS.includes(componentName)) {
          type = 'cards';
        } else {
          type = componentName;
        }
