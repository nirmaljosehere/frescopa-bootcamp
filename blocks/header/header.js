...
export default async function decorate(block) {
  ...
  // When content is served under a /content prefix (e.g. local dev), resolve the
  // nav fragment within that prefix; otherwise default to the site root.
  const contentPrefix = window.location.pathname.startsWith('/content/') ? '/content' : '';
  const navPath = navMeta ? new URL(navMeta, window.location).pathname : `${contentPrefix}/nav`;
  ...
  // The tools section is intentionally empty in the source document and may be
  // dropped by some authoring environments. Recreate it so the cart/search/account
  // tools always have a container and decoration never fails.
  if (!nav.querySelector('.nav-tools')) {
    const toolsSection = document.createElement('div');
    toolsSection.classList.add('section', 'nav-tools');
